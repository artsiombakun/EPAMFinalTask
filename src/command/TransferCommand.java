...
public class TransferCommand implements Command {
			...
			try {
				...
				if (isOwn) {
					if (DAOAccount.getInstance().transfer(from, to, sum)) {
						...
						request.setAttribute(Config.SUCCESS_ATTR, "Transfer has done!");
					} else
						request.setAttribute(Config.ERROR_ATTR,
								"Transfer failed!Caused by a)one/both accounts do not exist; "
										+ "b)one/both accounts are closed; c)you account has negative balance.");
				} else
					request.setAttribute(Config.ERROR_ATTR,
							"Transfer failed!You are not the owner of source account!");
	...
	private void reloadAccountsList(int id, HttpServletRequest request,
			HttpServletResponse response) throws ServletException, IOException,
			DAOException {
		...
		session.setAttribute(
				Config.ACC_LIST_TOTAL_PAGE_ATTR,
